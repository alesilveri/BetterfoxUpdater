...
def get_base_path():
    ...
    - In modalità compilata (--onefile): directory dove si trova l'eseguibile.
    - In modalità sviluppo: directory del file corrente.
    """
    if getattr(sys, 'frozen', False):
        # Applicazione compilata con Nuitka
...
def load_configs():
    """Carica config.ini da get_config_path."""
    config = configparser.ConfigParser()
    config_path = get_config_path()
    logging.info(f"Caricamento config da: {config_path}")
    if config_path.exists():
        ...
        logging.info(f"Config non trovato. Creazione di {config_path} con valori predefiniti.")
        config['Settings'] = {
            'profile_path': '',
            'theme': 'system',
            'backup_folder': ''
...
logging.basicConfig(
    ...
    level=logging.DEBUG  # Impostato su DEBUG per maggiori dettagli
)
...
def resource_path(relative_path):
    """
    Ottiene il percorso assoluto al file delle risorse.
    Funziona sia in modalità sviluppo che in modalità compilata (--onefile).
    
    :param relative_path: Percorso relativo alla cartella 'resources'.
    :return: Percorso assoluto al file delle risorse.
    """
    base_path = get_base_path()
    resource_full_path = base_path / 'resources' / relative_path
    logging.debug(f"Resource path for '{relative_path}': {resource_full_path}")
    return resource_full_path

RESOURCES_DIR = resource_path("")
ICON_PATH = resource_path("betterfox.ico")
ICON_PNG_PATH = resource_path("betterfox.png")
...
# ================================================================
config = load_configs()
# Se config.ini non esiste in nessuna posizione, creane uno con valori predefiniti
...
def set_theme(theme_name):
    ...
    # Applica il tema della barra del titolo immediatamente
    apply_title_bar_theme(root, dark=(style.theme_use() == 'darkly'))
    if ICON_PATH.exists():
...
if ICON_PATH.exists():
    try:
        if platform.system() == 'Windows':
            ...
            # Imposta l'icona della taskbar immediatamente
            set_taskbar_icon(root, str(ICON_PATH.resolve()))
        else:
            if ICON_PNG_PATH.exists():
                icon_image = tk.PhotoImage(file=str(ICON_PNG_PATH.resolve()))
                ...
                root.icon_image = icon_image  # Mantieni un riferimento all'immagine
                logging.debug(f"Iconphoto impostata su piattaforma non-Windows con: {ICON_PNG_PATH.resolve()}")
    except Exception as e:
    ...
    logging.warning("Icona non trovata nella cartella resources.")

# Definizione del menubar **prima** della chiamata a update_menu_style()
...
# Ora che menubar è definito, chiamiamo update_menu_style()
update_menu_style()
...
# Imposta il percorso del profilo Firefox
saved_path = config['Settings'].get('profile_path', '')
if saved_path and Path(saved_path).exists():
    profile_entry.insert(0, saved_path)
    logging.debug(f"Percorso del profilo Firefox caricato: {saved_path}")
else:
    ...
    if default_profile_path and default_profile_path.exists():
        ...
        save_configs(config)
        log_message([("ℹ️ ", None), (f"Percorso di default del profilo Firefox impostato su: {default_profile_path}", 'highlight')])
        logging.debug(f"Percorso di default del profilo Firefox impostato su: {default_profile_path}")
...
def update_main_info():
    ...
    if profile_path:
        ...
        if not profile_path.exists():
            ...
            logging.warning(f"Profilo specificato non esiste: {profile_path}")
            return
        firefox_version = get_firefox_version(profile_path)
        clear_log()
        if firefox_version:
            log_message([("🦊 ", None), ("Versione di Firefox installata: ", None), (firefox_version, 'highlight')])
            logging.info(f"Versione di Firefox installata: {firefox_version}")
        else:
            log_message([("❌ ", None), ("Impossibile rilevare la versione di Firefox.", None)])
            logging.warning("Impossibile rilevare la versione di Firefox.")
        local_version = get_local_version(profile_path)
        remote_version, remote_content = get_remote_version()
        last_update = get_github_last_update()
        ...
        if local_version:
            ...
            logging.info(f"Versione di Betterfox installata: v{local_version}")
        else:
            log_message([("💾 ", None), ("Betterfox non è installato nel tuo profilo.", None)])
            logging.info("Betterfox non è installato nel profilo.")

        if remote_version:
            log_message([("🌐 ", None), ("Ultima versione disponibile di Betterfox: ", None), (f"v{remote_version}", 'highlight')])
            logging.info(f"Ultima versione disponibile di Betterfox: v{remote_version}")
        else:
            log_message([("🌐 ", None), ("Ultima versione disponibile di Betterfox: ", None), ("Non disponibile", 'error')])
            logging.warning("Ultima versione disponibile di Betterfox non disponibile.")

        if last_update and last_update != "Data non disponibile":
            log_message([("🕒 ", None), ("Ultimo aggiornamento di Betterfox su GitHub: ", None), (last_update, 'highlight')])
            logging.info(f"Ultimo aggiornamento su GitHub: {last_update}")
        else:
            log_message([("🕒 ", None), ("Ultimo aggiornamento di Betterfox su GitHub: ", None), ("Non disponibile", 'error')])
            logging.warning("Ultimo aggiornamento di Betterfox su GitHub non disponibile.")

# ================================================================
# AVVIO DELLE CONFIGURAZIONI E DELLA GUI
# ================================================================
def initialize_app():
    # Applica immediatamente il tema della barra del titolo
    apply_title_bar_theme(root, dark=(style.theme_use() == 'darkly'))
    # Aggiorna le informazioni principali nella GUI
    update_main_info()
    logging.debug("Applicazione inizializzata.")

# Avvia l'inizializzazione dell'app dopo che la GUI è pronta
