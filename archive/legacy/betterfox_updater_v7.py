...
===============================================================================
v2.3 (2025-06-10)
  - Miglior gestione icona in modalità onefile (PyInstaller/Nuitka) tramite resource_path.
  - Pulizia delle funzioni obsolete di ricerca icona.
  - Piccole rifiniture logging e packaging.
"""
# =============================================================================
...
# =============================================================================
# FUNZIONE GENERICA PER RISORSE (dev vs frozen)
# =============================================================================
def resource_path(rel_path: str) -> Path:
    """
    Restituisce il Path della risorsa rel_path:
    - in dev: cerca in {script_dir}/rel_path, altrimenti in {script_dir.parent}/rel_path
    - in frozen (PyInstaller/Nuitka):
      * PyInstaller: usa sys._MEIPASS
      * Nuitka: Path(sys.executable).parent
    """
    if getattr(sys, 'frozen', False):
        ...
        if not candidate.exists():
            # prova anche nella cartella padre
            parent_candidate = base.parent / rel_path
...
def find_icon_paths():
    """
    Cerca .ico e .png in resources tramite resource_path.
    Restituisce tuple (ico_path or None, png_path or None).
    """
    ico_path = None
    png_path = None
    res_dir = resource_path("resources")
    if not (res_dir.exists() and res_dir.is_dir()):
        # in dev, potrebbe trovarsi in ../resources
        alt = Path(__file__).resolve().parent.parent / "resources"
        if alt.exists() and alt.is_dir():
            res_dir = alt
    if res_dir.exists() and res_dir.is_dir():
        for f in res_dir.iterdir():
            if f.is_file() and f.suffix.lower() == ".ico" and ico_path is None:
                ico_path = f
            if f.is_file() and f.suffix.lower() == ".png" and png_path is None:
                png_path = f
    return ico_path, png_path


def set_taskbar_icon(window, icon_path: Path):
    """
    Su Windows, imposta icona della taskbar/titlebar via ctypes LoadImageW.
    """
    if platform.system() != 'Windows':
        return
    try:
        ...
        # WM_SETICON = 0x80, 0 = small, 1 = big
        ctypes.windll.user32.SendMessageW(hwnd, 0x80, 0, hicon)
        ctypes.windll.user32.SendMessageW(hwnd, 0x80, 1, hicon)
    except Exception as e:
        logging.error(f"Errore set_taskbar_icon: {e}")

def apply_icon_to_window(win: tk.Tk or tk.Toplevel):
    """
    Applica l'icona principale alla finestra win, usando resource_path.
    """
    ico_path, png_path = find_icon_paths()
    if ico_path and ico_path.exists():
        try:
            if platform.system() == "Windows":
                win.iconbitmap(str(ico_path))
                set_taskbar_icon(win, ico_path)
            else:
                if png_path and png_path.exists():
                    img = tk.PhotoImage(file=str(png_path))
                    win.iconphoto(True, img)
                    # mantenere riferimento per evitare garbage collection
                    win.icon_image = img
        except Exception as e:
            logging.error(f"Errore impostazione icona finestra: {e}")
    else:
        logging.warning(f"Icona non trovata a runtime in {resource_path('resources')}")

def apply_title_bar_theme(window, dark: bool):
    """
    Applica il tema scuro/chiaro alla title bar di Windows, se disponibile.
    Deve essere definita prima di ogni chiamata.
    """
    if platform.system() != "Windows":
        return
    try:
...
# Aggiungi anche console handler in dev se serve:
...
def get_local_version(profile_path: Path):
...
def get_firefox_version(profile_path=None):
    ...
    Recupera la versione di Firefox:
      ...
      - Su Windows/Linux/macOS path/comandi
    """
...
def get_default_firefox_profile():
    ...
    system_os = platform.system()
    if system_os == 'Windows':
        base_dir = Path(os.environ.get('APPDATA', '')) / 'Mozilla' / 'Firefox'
    elif system_os == 'Darwin':
    ...
    # Ultimo profilo usato tramite times.json
...
# =============================================================================
def is_dark_theme():
    """Determina se usare il tema scuro (darkdetect o impostazione)."""
    theme = config['Settings'].get('theme', 'system')
    if theme == 'system':
        return darkdetect.isDark()
    elif theme in ('dark', 'darkly'):
        return True
    else:
        return False

# =============================================================================
# FUNZIONI LOG INTERFACCIA
# =============================================================================
last_message = ""
messages_displayed = False
after_id = None
progress_bar_widget = None  # assegnato in init

def log_message(message_parts, clear=False):
    """
    Inserisce testo nel widget di log. 
    'message_parts' è lista di tuple (testo, tag).
    ...
    for part, tag in message_parts:
        # aggiungo spazio dopo emoji iniziale
        if first and part and part[0] in ["🦊","💾","🌐","🕒","🗂️","❌","✅","🔄","⚠️","📥","🎉","ℹ️"]:
...
def finish_update():
    """Ripristina UI dopo operazione: abilita pulsanti, nasconde progress bar, torna a main info."""
    enable_buttons()
    hide_progress_bar()
    return_to_main_info()
...
def compress_backup_folder(backup_path: Path):
    """
...
def handle_old_backup(backup_path: Path):
    """
...
def fallback_copy_tree(src: Path, dst: Path):
...
def create_full_backup(profile_path: Path):
    """
...
def restore_backup_simple(profile_path: Path):
...
def show_progress_bar():
    ...
    global progress_bar_widget
    if progress_bar_widget:
        progress_bar_widget.grid(row=2, column=0, columnspan=3, pady=5, sticky="ew")
...
def hide_progress_bar():
    ...
    if progress_bar_widget:
        ...
        progress_bar_widget.grid_remove()
...
def download_userjs(profile_path: Path, remote_content: str, remote_version: str):
...
def run_update(profile_path_str: str):
    """Avvia l'aggiornamento in un thread, mostra la progress bar e disabilita pulsanti."""
    disable_buttons()
    clear_log()
    show_progress_bar()
    threading.Thread(target=run_update_thread, args=(Path(profile_path_str),), daemon=True).start()

def run_update_thread(profile_path: Path):
    """
...
# =============================================================================
# GESTIONE PROFILO E CONFIG
# =============================================================================
def save_profile_path(path: str):
    """Salva il percorso del profilo in config.ini."""
    config['Settings']['profile_path'] = str(path)
    save_configs(config)
...
def restore_backup(profile_path: Path):
    """Ripristina user.js da un backup semplice selezionato."""
    clear_log()
    log_message([("🔄", None), ("Seleziona un backup semplice da ripristinare...", None)])
    backup_to_restore = filedialog.askdirectory(title="Seleziona un backup da ripristinare")
    if backup_to_restore:
        ...
        for file_name in ["user.js"]:
            ...
            if backup_file.exists():
                try:
                    copy2(backup_file.resolve(), (Path(profile_entry.get()) / file_name).resolve())
                    log_message([("✅", None), (file_name, 'highlight'), (" ripristinato con successo.", None)])
                except Exception as e:
                    log_message([("❌", None), ("Errore ripristino ", None), (file_name, 'highlight'), (": ", None), (str(e), 'error')])
    else:
...
root = tk.Tk()
root.withdraw()
root.title("Betterfox Updater v2.3")
root.geometry("500x420")
root.resizable(False, False)

# Applica icona finestra principale
apply_icon_to_window(root)
...
options_menu.add_command(label="Ricarica Informazioni", command=return_to_main_info)
...
progress_bar_widget = ttk.Progressbar(profile_labelframe, orient="horizontal", mode="indeterminate")
progress_bar_widget.grid_remove()

# Footer
footer = ttk.Label(root, text="Betterfox Updater v2.3")
footer.grid(row=2, column=0, pady=(5,10))

style.configure('TButton', focusthickness=0)

# Imposta profilo salvato o default
...
def initialize_app():
    """Applica tema, icona, aggiorna info e mostra la finestra."""
    apply_title_bar_theme(root, dark=is_dark_theme())
    apply_icon_to_window(root)
    update_main_info()
    update_colors_based_on_theme()
