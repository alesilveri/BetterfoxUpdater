...
def get_base_path():
    if getattr(sys, 'frozen', False):
        # Quando l'applicazione è compilata, usa AppData
        appdata = os.getenv('APPDATA')
        if appdata:
            return Path(appdata) / 'BetterfoxUpdater'
        else:
            # Fallback se APPDATA non è definito
            return Path.home() / 'BetterfoxUpdater'
    else:
...
def resource_path(relative_path):
    if getattr(sys, 'frozen', False):
        # Quando l'applicazione è compilata, le risorse sono nella stessa directory dell'eseguibile
