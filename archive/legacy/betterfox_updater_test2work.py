...
def get_base_path():
    """
    In modalità 'frozen' (Nuitka --onefile), vogliamo puntare
    alla cartella DOVE RISIEDE L'ESEGUIBILE vero e proprio,
    così da creare/leggere config.ini lì.

    In modalità sviluppo, assumiamo la struttura:
        Progetto/
          resources/
          src/
            betterfox_updater_test2.py
    e puntiamo alla cartella 'Progetto'.
    """
    if getattr(sys, 'frozen', False):
        # Con Nuitka --onefile, sys.executable in realtà punta
        # alla cartella di estrazione in %TEMP%.
        # Ma con --onefile-tempdir=., verrà estratto ACCANTO all'exe
        # e sys.argv[0] è il path dell'eseguibile nella cartella reale.
        exe_path = Path(sys.argv[0]).resolve()
        return exe_path.parent
    else:
        # Sviluppo: cartella padre di 'src'
        return Path(__file__).resolve().parent.parent

def get_config_path():
    """Percorso di config.ini nella directory di esecuzione (vedi get_base_path)."""
...
def find_ico_in_resources():
    ...
    if not resources_dir.exists() or not resources_dir.is_dir():
        logging.warning("Cartella resources non trovata.")
        return None
    ...
    for file in resources_dir.iterdir():
        if file.is_file() and file.suffix.lower() == ".ico":
            ...
            return file
    logging.warning("Icona .ico non trovata nella cartella resources.")
    return None
