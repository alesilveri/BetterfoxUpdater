...
def run_update(profile_path):
    ...
    clear_log()
    show_progress_bar()
    threading.Thread(target=run_update_thread, args=(Path(profile_path),), daemon=True).start()
...
# =============================================================================
# Impostiamo l'icona nel codice di inizializzazione, non qui
# =============================================================================
...
def initialize_app():
    """Applica tema, imposta icona, aggiorna info e mostra la finestra (deiconify)."""
    apply_title_bar_theme(root, dark=is_dark_theme())
    ...
    try:
        if ICON_PATH:
            if platform.system() == 'Windows':
                ...
                # icona per taskbar/dock (PNG se disponibile)
                if ICON_PNG_PATH:
                    icon_image = tk.PhotoImage(file=str(ICON_PNG_PATH.resolve()))
        ...
        logging.error(f"Errore caricamento icona principale in initialize_app: {e}")
